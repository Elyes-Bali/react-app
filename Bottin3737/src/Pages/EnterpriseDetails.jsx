...
function EnterpriseDetails() {
  ...
  async function getComments() {
    ...
    querySnapshot.forEach((doc) => {
      if (doc.exists()) {
        ...
        console.log('No such document with ID: ');
      }
    });
  ...
  async function sendComment() {
    ...
    let ref = doc(collection(db, 'comments'));
    let data = {
      ...commentDetails,
      company: userInfo.companyName,
      status: 'null',
    ...
    try {
      await setDoc(ref, data).then(() => {
        console.log('Comment sended successfully ');
        setCommentDetails({
          comment: '',
          company: userInfo.companyName
        });
        toast('Votre commentaire a été envoyée avec succès');
        alert('Votre commentaire a été envoyée avec succès');
      });
    } catch (error) {
      notifyError();
                ...
                >
                  <Box style={{ flex: 3, padding: '30px', margin: '0px 10px' }}>
                    ...
                    >
                      ...
                      >
                        ...
                        {isVisible ? (
                          ...
                          <p className="pl-2">Press on me to see the phone number</p>
                        )}
                      </Box>
                    ...
                    <Box>
                      ...
                      {Object.values(userComment).map((comment, index) => {
                        console.log('comment:', comment);
                        console.log('userInfo.companyName:', userInfo.companyName);
                        if (comment?.company === userInfo.companyName) {
                          console.log('Rendering comment:', comment);
                          return (
                            <Typography key={index} style={{ margin: '10px 5px' , backgroundColor:'white',borderRadius: 15,}}>
                              {comment?.comment}
                            </Typography>
                          );
                        }
